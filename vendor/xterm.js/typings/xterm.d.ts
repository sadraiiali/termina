...
declare module 'xterm' {
  ...
  export interface ITerminalOptions {
    ...
    /**
     * Enable right-to-left line reordering, bracket mirroring (UBA L4), and
     * visual→logical selection mapping for Persian/Arabic/Hebrew. Inspired by
     * iTerm2 PR #709. Default false (inert).
     */
    bidi?: boolean;

    /**
     * When `bidi` is on, treat Latin letter runs as LTR islands in RTL
     * paragraphs so paths and identifiers keep order. Default false.
     */
    bidiLatinIslands?: boolean;

    /**
     * When `bidi` is on, strip zero-width format characters (ZWNJ, LRM, …)
     * on paste. Default false.
     */
    bidiStripZwOnPaste?: boolean;

    /**
     * When `bidi` is on, draw guillemets «» ‹› as typed rather than
     * bidi-mirrored. Default true.
     */
    bidiKeepGuillemetsAsTyped?: boolean;
