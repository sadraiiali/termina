...
import { analyzeCells } from 'common/bidi/Bidi';
...
export class SelectionService extends Disposable implements ISelectionService {
  ...
  private _getMouseBufferCoords(event: MouseEvent): [number, number] | undefined {
    ...
    // Visual column → logical cell (iTerm2 PR #709 selection model)
    if (this._optionsService.rawOptions.bidi) {
      coords[0] = this._visualToLogicalX(coords[0], coords[1]);
    }
    return coords;
  }

  /**
   * Map a visual column on a buffer line to the logical cell index.
   * Identity when bidi is off or the line has no RTL reordering.
   */
  private _visualToLogicalX(visualX: number, bufferY: number): number {
    const line = this._bufferService.buffer.lines.get(bufferY);
    if (!line) {
      return visualX;
    }
    const lineLength = Math.min(line.getNoBgTrimmedLength() || this._bufferService.cols, this._bufferService.cols);
    if (visualX < 0 || visualX >= this._bufferService.cols) {
      return visualX;
    }
    // Past end of content: leave alone (iTerm2 boundary rule)
    if (visualX >= lineLength) {
      return visualX;
    }
    const cellChars: string[] = [];
    const work = new CellData();
    for (let i = 0; i < lineLength; i++) {
      line.loadCell(i, work);
      cellChars.push(work.getChars() || ' ');
    }
    const info = analyzeCells(cellChars, {
      enabled: true,
    ...
    if (visualX >= 0 && visualX < info.logicalForVisual.length) {
